"use client";

import Link from "next/link";
import { motion } from "framer-motion";

const fadeUp = {
  hidden: { opacity: 0, y: 40 },
  show: (i: number) => ({
    opacity: 1,
    y: 0,
    transition: { delay: i * 0.08, duration: 0.6 },
  }),
};

export default function WhatsAppClient() {
  return (
    <main className=" p-6 min-h-screen bg-[#0b1220] text-white overflow-hidden">

      {/* HERO */}
      <motion.section
        initial={{ opacity: 0, scale: 0.95 }}
        animate={{ opacity: 1, scale: 1 }}
        className="relative p-8 rounded-2xl bg-gradient-to-r from-green-500 to-blue-500 overflow-hidden"
      >
        {/* Glow */}
        <div className="absolute w-72 h-72 bg-white/30 blur-[120px] top-0 left-0 animate-pulse"></div>

        <h1 className="text-3xl md:text-4xl font-bold">
          WhatsApp API Integration Services 💬
        </h1>

        <p className="mt-3 text-white/90 max-w-2xl">
          Automate your business communication with WhatsApp API,
          chatbots, auto-replies and real-time notifications.
        </p>

        <Link
          href="/contact"
          className="inline-block mt-5 bg-white text-black px-5 py-2 rounded-lg font-semibold hover:scale-105 transition"
        >
          Get Free Setup
        </Link>
      </motion.section>

      {/* WHY */}
      <section className="mt-12">
        <motion.h2
          variants={fadeUp}
          initial="hidden"
          whileInView="show"
          className="text-2xl font-bold"
        >
          Why WhatsApp API?
        </motion.h2>

        <div className="grid md:grid-cols-2 gap-5 mt-6">
          {[
            "Instant customer communication",
            "Automated replies & chatbot",
            "Increase sales conversions",
            "24/7 customer support system",
          ].map((item, i) => (
            <motion.div
              key={i}
              custom={i}
              variants={fadeUp}
              initial="hidden"
              whileInView="show"
              whileHover={{ scale: 1.05 }}
              className="p-5 bg-white/5 border border-white/10 rounded-xl"
            >
              ✔ {item}
            </motion.div>
          ))}
        </div>
      </section>

      {/* FEATURES */}
      <motion.section
        initial="hidden"
        whileInView="show"
        variants={fadeUp}
        className="mt-12 p-6 rounded-xl bg-white/5 border border-white/10"
      >
        <h2 className="text-2xl font-bold">
          Features We Provide
        </h2>

        <ul className="mt-4 space-y-2 text-gray-300">
          <li>✔ WhatsApp Business API Setup</li>
          <li>✔ Auto Reply System</li>
          <li>✔ Chatbot Integration</li>
          <li>✔ Bulk Messaging System</li>
          <li>✔ Order & Notification Alerts</li>
        </ul>
      </motion.section>

      {/* USE CASES */}
      <section className="mt-12">
        <h2 className="text-2xl font-bold">
          Use Cases
        </h2>

        <div className="grid md:grid-cols-3 gap-6 mt-6">
          {[
            {
              title: "E-commerce",
              desc: "Order updates & customer support automation.",
            },
            {
              title: "Education",
              desc: "Student updates & automated messaging.",
            },
            {
              title: "Business",
              desc: "Lead follow-ups & engagement automation.",
            },
          ].map((item, i) => (
            <motion.div
              key={i}
              custom={i}
              variants={fadeUp}
              initial="hidden"
              whileInView="show"
              whileHover={{ scale: 1.07 }}
              className="p-6 bg-white/5 border border-white/10 rounded-xl"
            >
              <h3 className="font-bold text-lg">{item.title}</h3>
              <p className="text-gray-400 text-sm mt-2">
                {item.desc}
              </p>
            </motion.div>
          ))}
        </div>
      </section>

      {/* BENEFITS */}
      <section className="mt-12">
        <motion.h2
          variants={fadeUp}
          initial="hidden"
          whileInView="show"
          className="text-2xl font-bold"
        >
          Benefits of WhatsApp Automation
        </motion.h2>

        <div className="grid md:grid-cols-2 gap-5 mt-6">
          {[
            "Save time with automation",
            "Increase customer engagement",
            "Boost sales conversions",
            "Improve customer experience",
          ].map((item, i) => (
            <motion.div
              key={i}
              custom={i}
              variants={fadeUp}
              initial="hidden"
              whileInView="show"
              whileHover={{ scale: 1.05 }}
              className="p-5 bg-white/5 border border-white/10 rounded-xl"
            >
              ✔ {item}
            </motion.div>
          ))}
        </div>
      </section>

      {/* CTA */}
      <motion.section
        initial={{ opacity: 0, scale: 0.9 }}
        whileInView={{ opacity: 1, scale: 1 }}
        className="mt-16 text-center p-10 rounded-2xl bg-gradient-to-r from-green-500 to-blue-500"
      >
        <h2 className="text-3xl font-bold">
          Automate Your Business on WhatsApp 🚀
        </h2>

        <p className="mt-3 text-white/90">
          Start using WhatsApp API and grow your business faster.
        </p>

        <Link
          href="/contact"
          className="inline-block mt-6 bg-white text-black px-6 py-3 rounded-lg font-semibold hover:scale-110 transition"
        >
          Get Started
        </Link>
      </motion.section>

    </main>
  );
}